      ...
      {user && (
        <div className='absolute bottom-2 left-2 flex items-center gap-2 bg-gray-900/90 px-3 py-2 rounded-lg backdrop-blur-sm border border-white/20'>
          ...
          <div className='flex flex-col'>
            ...
            <span
              className={`text-xs ${
                isLocal ? 'text-green-400' : 'text-blue-400'
              }`}
            >
...
export const VideoGrid: React.FC<VideoGridProps> = ({
  ...
  const hasSomeone = remoteStreams.length > 0 || activeCameraStreams.size > 0;
  ...
  return (
    <div className='relative h-full w-full'>
      ...
      {/* Multiple camera streams - Grid en esquina inferior izquierda */}
      {activeCameraStreams.size > 0 && (
        <div className='absolute bottom-4 left-4 z-10 flex flex-col gap-2'>
          {Array.from(activeCameraStreams.entries()).map(
            ([deviceId, stream], index) => (
              <div
                key={`camera-${deviceId}`}
                className='relative w-48 h-36 shadow-2xl border-2 border-blue-500/50 rounded-lg overflow-hidden'
              >
                <VideoContainer
                  stream={stream}
                  user={localUser}
